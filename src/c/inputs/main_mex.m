...
P = zeros(3,Np);
P(:,1) = [sum(p(1,:)) sum(p(2,:)) sum(p(3,:))]'/Np;
fig = 1;
...
save('set_value.mat', 'w', 'p', 'L', 'N', 'R','alpha','map', 'max_range','angles','mapscale', 'P')
...
%% particle filter
    ...
    vec_dtick_L(k) = dtick_L;
    vec_dtick_R(k) = dtick_R;
%     save(sprintf('k_%02d',k), 'p', 'CovSonars', 'w', 'P');
%% animation
...
save('dynamic_param.mat', 'vec_dtick_L', 'vec_dtick_R', 'sonars','see_sonar', 'CovSonars')
save('final.mat', 'x', 'y', 'th', 'see_sonar', 'odometry')
