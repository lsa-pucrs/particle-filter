...
mex mexRayCast.c
mex mexRayCasting.c
mex mexEstimateProb.c
mex mexSamplingWheel.c
...
%% ray casting   
   sonars = mexRayCast(x(k+1),y(k+1),th(k+1),map_trans,max_range,angles,mapscale,1);
   sonars = sonars/mapscale;
...
%% particle filter
   for i = 1:Np
       ...
       p(:,i) = mexEstimateProb(p(:,i),dtick_L,dtick_R,L,N,R,alpha);                
       ...
       if map(max(1,min(ceil(p(2,i)*mapscale),size(map,1))),max(1,min(ceil(p(1,i)*mapscale),size(map,2))))   
           ...
           % simulate the sonars for the particles and calculate the weight
           w(i) = mexRayCasting(p(1,i),p(2,i),p(3,i),map_trans,max_range,angles,mapscale,1,sonars,CovSonars,w(i));
       end
   end   
   w = w/sum(w);
