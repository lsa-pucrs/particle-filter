...
void mexRayCast1(double xc, double yc, double theta, double *map, mwSize map_M, mwSize map_N, double max_range, double *angles, mwSize nAngles, mwSize mapscale, double accuracy, double *range)
{
    /* Code for ray cast */
    mwSize i, end_flag = 0;
    ...
    if((yc > map_N) || (xc > map_M) || (xc <= 0) || (yc <= 0)){
        for(i=0; i<nAngles; i++)
            range[i] = 0;
        end_flag = 1;
    }
                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    mexRayCast1(x_t, y_t, theta_t, map_t, map_M, map_N, max_range_t, angles_t, nAngles, mapscale_t, accuracy_t, range_t);
}
