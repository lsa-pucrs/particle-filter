...
p = fdraw_p(length(p),2,2,10,map,x+1,y+0.5,th,mapscale);
...
%% particle filter
   ...
   % sampling wheel
   if rem(k,15)==1 && k>10 % choose "-1" for open loop simulation of the Part. Filt
     beta = 0;
...
%% animation
    if 1 %to anime or not to anime        
        F_anima_map([x(k+1)*mapscale y(k+1)*mapscale th(k+1)], L*mapscale, fig, map,sonars*mapscale,angles)                   
        for i = 1:size(p,2)       
            % car pose
            plot(p(1,i)*mapscale,p(2,i)*mapscale,'k.')    
            %car nose
            plot([p(1,i) p(1,i)+0.05*cos(p(3,i))]*mapscale,[p(2,i) p(2,i)+ 0.05*sin(p(3,i))]*mapscale,'k');
        end     
        F=getframe(gca);
        mov = addframe(mov,F);
        pause(eps)
